...
test("題材語だけでは設問にない評価類型を持ち込まない", () => {
  ...
  assert.deepEqual(detectEssayTaskTypes("少子化問題への賛否を論じなさい。"), []);
  assert.deepEqual(detectEssayTaskTypes("計画経済の功罪について論じなさい。"), []);
});
...
test("類型を示す名詞だけでは処理要求を持ち込まない", () => {
  assert.deepEqual(detectEssayTaskTypes("数値目標を設けることの是非を論じなさい。"), []);
  assert.deepEqual(detectEssayTaskTypes("欧州統合の功罪を論じなさい。"), []);
  assert.deepEqual(detectEssayTaskTypes("公文書館における資料保存の意義を論じなさい。"), []);
  assert.deepEqual(detectEssayTaskTypes("政策を提案することの功罪を論じなさい。"), []);
});
...
test("題材中の動詞や一般的な意見表明を処理要求と誤認しない", () => {
  assert.deepEqual(
    detectEssayTaskTypes("あなた自身は死刑制度に賛成か、理由とともに論じなさい。"),
    []
  );
  assert.deepEqual(detectEssayTaskTypes("AIが計算することの功罪を論じなさい。"), []);
