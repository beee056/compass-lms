...
export default function EditDocumentDialog({ document, universities = [], trigger }: EditDocumentDialogProps) {
  ...
  const [universityId, setUniversityId] = useState(document.universityId ?? "");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const result = await updateDocument(document.id, title, dueDate || null, universityId || null);
      if (result.success) {
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid gap-4 py-4">
            ...
            {universities.length > 0 && (
              <div className="grid gap-2">
                <Label htmlFor="docUniversity" className="text-slate-700 font-semibold text-sm">対象の志望校（タブの振り分け）</Label>
                <select
                  id="docUniversity"
                  value={universityId}
