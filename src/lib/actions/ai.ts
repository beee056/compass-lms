...
interface RubricEvaluationOptions {
  ...
  // 自己分析・将来ビジョンを文脈として参照する生徒（志望理由書・面接で本人に即した助言に使う）
  selfProfileStudentId?: string;
}
...
${questionDependentAxes.map((axis) => `- 「${axis.label}」: ${axis.questionDependent!.guidance}`).join("\n")}`
    ...
    // 自己分析・将来ビジョンの文脈（志望理由書・面接でのみ、本人に即した助言のために参照）
    let selfProfileContext = "";
    if ((kind === "志望理由書" || kind === "面接") && options?.selfProfileStudentId) {
      const selfProfile = await prisma.studentSelfProfile.findUnique({
        where: { studentProfileId: options.selfProfileStudentId }
      });
      selfProfileContext = buildSelfProfileContext(selfProfile);
    }
...
export async function evaluateWithRubric(
  ...
  try {
    ...
    const { authoritativePromptText, totalScore, feedbackPayload } = await runRubricEvaluation(
      ...
      promptText,
      answer,
      { ...options, previousTurn, usedFollowUps, followUpCandidates, selfProfileStudentId: studentId }
    );
...
export async function evaluatePracticeInstant(params: {
  ...
  try {
    ...
    // 生徒本人がインスタント添削を使う場合は、自分の自己分析を文脈に含める
    const ownStudentId =
      user.role === "STUDENT" ? user.studentProfile?.id ?? null : null;
    ...
    const { authoritativePromptText, totalScore, feedbackPayload } = await runRubricEvaluation(
      ...
      params.answer,
      {
        ...
        selfProfileStudentId: ownStudentId ?? undefined
      }
    );
